...
class Char(namedtuple("Char", [     #创建一个char类结构，保存每一个字符在屏幕中显示的参数。看不懂怎么赋值以及下面new函数是怎么使用的
    "data",                           #namedtuple 结构如果创建后就无法进行修改，后期修改是会报错的注意！
    "fg",
    "bg",
    "bold",
    "italics",
...
class Screen(object):
    ...
    def erase_in_display(self, how=0, private=False):
        ...
        for y in interval:          
            line = self.buffer[y]    #这里buffer[y]中的数据是一个自定义的数据结构-字典  一个line中数据类似如下：
...
## 3: Char(data='4', fg='default', bg='default', bold=False, italics=False, underscore=False, strikethrough=False, reverse=False)}
            ...
            for x in line:
                line[x] = self.cursor.attrs  #这里将 字典key对应的值设置为默认，data为空
