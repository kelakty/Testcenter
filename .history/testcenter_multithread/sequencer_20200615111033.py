    ...
    def sequence_table(self, test_sequence_list):
        ...
        for test_info in test_sequence_list:
            ...
            if int(test_info["是否测试"]) != False:   #表格读取到的是字符型，需要转换为整形
                ...
                #发送指令不为空才发送
                if test_info["发送指令"] != "" or test_info["发送指令"] != "nan":
                ...
                #输出本次结果到报告
                self.report = pd.read_excel(GlobalVariable.sequencer_choosefilename, sheet_name = '机框式交换机生测checklist')
                print(self.report)
                print("C行值",self.report.loc[:, "C" ].values)
                print("本次测试结果为：",self.report[5:"D"])

        self.report = GlobalVariable.sequencer_choosefilename+"_%d%02d%02d_%d_%02d_%02d"% (date.year, date.month, date.day,time.hour,time.minute,time.second)+".xlsx"
