...
class MainWindow(QMainWindow, Ui_MainWindow, MyUi_MainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.main_logger.info('开始记录主线程log...')  #TODO 暂时关闭log
        # self.crt=CRT()
    ...
    def on_action12_X_factory_auto_test_triggered(self):
        ...
        if self.mdiArea.currentSubWindow() != None:
            ...
            if self.current_console_index >= 0:
                ...
                # self.fac_auto_test_threadpool.finished.connect(self.fac_auto_test_threadpool.deleteLater)
                ...
                self.fac_auto_test_threadpool.exec()
                print("启动线程...")


    def fac_auto_test_finished(self):
        self.current_console_index = self.find_dictionarylist_keyvalue_index(GlobalVariable.Console, "subwindowobj", self.mdiArea.currentSubWindow())
        GlobalVariable.Console[self.current_console_index]["consolethread"].rec_trigger.disconnect(self.sequencer_receive_data_slot)
        self.fac_auto_test_thread.fac_test_to_main_trigger.disconnect(self.sequencer_data_to_console_thread_slot) 
        self.fac_auto_test_threadpool.requestInterruption()
        self.fac_auto_test_threadpool.quit()
        self.fac_auto_test_threadpool.exit()
        
        # self.fac_auto_test_threadpool.term
        # self.fac_auto_test_threadpool.wait()
        self.fac_auto_test_thread.deleteLater  #不能加括号
...
#######Sequencer测试序列功能########
    ...
    def on_run_test_sequence_clicked(self):
        ...
        print("当前子窗口是：",self.mdiArea.currentSubWindow())
        if self.mdiArea.currentSubWindow() != None:
            #读取当前的sequence table表格
            tablelist_header = []
            
            self.tablelist = [[0 for i in range(self.sequencertable.columnCount())] for j in range(self.sequencertable.rowCount())]
            for i in range(self.sequencertable.rowCount()):
                for j in range(self.sequencertable.columnCount()):
                    print("表格内容",self.sequencertable.item(i,j).text())
                    self.tablelist[i][j] = self.sequencertable.item(i,j).text()
            print("tablelist is :", self.tablelist)
            ...
            print("表格头列表:",tablelist_header)
            print(len(self.tablelist))
            for i in range(len(self.tablelist)):
                table_oneline_dict = self.two_list_convert_to_keyvalue_dict(tablelist_header,self.tablelist[i])
                GlobalVariable.table_dict_list.append(table_oneline_dict)
            print("table_dict_list",GlobalVariable.table_dict_list)
            ...
            if self.current_console_index >= 0:
                ...
                # # sequencer_thread = SequencerThreadWorker(GlobalVariable.Console[self.current_console_index]["consolethread"])   #需要传入当前激活的终端名
                # sequencer_thread = SequencerThreadWorker() 
                # sequencer_thread.moveToThread(self.sequencer_threadpool[-1])
                # # self.sequencer_threadpool[-1].started.connect(lambda: sequencer_thread.sequencer_worker(GlobalVariable.table_dict_list))
                # self.sequencer_threadpool[-1].started.connect(sequencer_thread.sequencer_worker)
                # sequencer_thread.seq_to_main_trigger.connect(self.sequencer_data_to_console_thread_slot, Qt.QueuedConnection)  #
                # sequencer_thread.main_to_seq_trigger.connect(sequencer_thread.received_console_log)
                # self.sequencer_threadpool[-1].start()
                # #新建sequencer线程
                # self.current_consolethread = GlobalVariable.Console[self.current_console_index]["consolethread"]
                self.sequencer_threadpool = QThread()
                self.sequencer_thread = SequencerThreadWorker(GlobalVariable.Console[self.current_console_index]["consolethread"])   #需要传入当前激活的终端名
                # self.sequencer_thread = SequencerThreadWorker()
                self.sequencer_thread.moveToThread(self.sequencer_threadpool)
                # self.sequencer_threadpool.started.connect(lambda: self.sequencer_thread.sequencer_worker(table_dict_list))
    ...
    def sequencer_data_to_console_thread_slot(self, current_consolethread, data):
        ...
        if type(data) == bytes:
            current_consolethread.send_trigger.emit(data)
        else:
    ...
    def on_import_to_sequencer_clicked(self):
        ...
        try:
            ...
            self.sequencer_choosefilename, _ = QFileDialog.getOpenFileName(self, "选取文件",QStandardPaths.standardLocations(0)[0],
                                "Excel Files (*.xlsx *.xls);;All Files (*)")
            print(self.sequencer_choosefilename)
    ...
    def two_list_convert_to_keyvalue_dict(self,columnname,cellitem):
        ...
        使用两个列表，生成一一对应的字典
        Args:
            columnname : 列名称的list,即字典的key
            cellitem ： 列值的list，即字典的values
