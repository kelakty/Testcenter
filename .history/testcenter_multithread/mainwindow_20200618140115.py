...
#######Sequencer测试序列功能########
    ...
    def on_export_sequence_table_clicked(self):
        ...
        """
        #读取当前表格
        for i in range(len(self.sequencertable.rowCount())):
            for j in range(len(self.sequencertable.columnCount())):
                self.test_table.loc[0+i,6+j]=self.sequencertable.item(i,j)
