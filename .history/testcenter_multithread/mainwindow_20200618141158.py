...
#######Sequencer测试序列功能########
    ...
    def on_export_sequence_table_clicked(self):
        ...
        for i in range(self.sequencertable.rowCount()):
            for j in range(self.sequencertable.columnCount()):
                self.test_table.loc[0+i,6+j]=self.sequencertable.item(i,j).text()
        print("导出表格为：",self.test_table)
        self.export_name = GlobalVariable.sequencer_choosefilename+"_%d%02d%02d_%d_%02d_%02d"% (datetime.now().year, datetime.now().month, datetime.now().day,datetime.now().hour,datetime.now().minute,datetime.now().second)+".xlsx"
        self.export_writer = pd.ExcelWriter(self.export_name, engine='xlsxwriter')
        self.test_table.to_excel(self.export_writer, sheet_name='测试用例')
