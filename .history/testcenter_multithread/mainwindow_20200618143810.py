...
#######Sequencer测试序列功能########
    ...
    def import_file_to_sequencer(self,choosefilename):
        if choosefilename:
            ...
            self.test_sequence = self.test_table[["测试项",'是否测试',"终端名","发送指令","等待回显时间","需匹配文本","不能匹配到文本","执行完是否清空log缓存","单独保存log到文件","生成报告文件","备注"]]
            print("测试序列表格是：\r\n",self.test_sequence)
    ...
    def on_export_sequence_table_clicked(self):
        ...
        for i in range(self.sequencertable.rowCount()):
            # for j in range(self.sequencertable.columnCount()):
            self.test_table[i,"是否测试"] = self.sequencertable.item(i,0).text()
            self.test_table[i,"终端名"] = self.sequencertable.item(i,1).text()
            self.test_table[i,"发送指令"] = self.sequencertable.item(i,2).text()
            self.test_table[i,"等待回显时间"] = self.sequencertable.item(i,3).text()
            self.test_table[i,"需匹配文本"] = self.sequencertable.item(i,4).text()
            self.test_table[i,"不能匹配到文本"] = self.sequencertable.item(i,5).text()
            self.test_table[i,"执行完是否清空log缓存"] = self.sequencertable.item(i,6).text()
            self.test_table[i,"单独保存log到文件"] = self.sequencertable.item(i,7).text()
            self.test_table[i,"备注"] = self.sequencertable.item(i,8).text()
