...
class MultiTree(object):
    ...
    def __init__(self, tree_root_name,command):
        self.count = 0
        self.tree = TreeNode(tree_root_name,command)
    ...
    def if_node_name_exist_recursion(self, tree, node_name, search):
        '''
        tree:需要判断是否存在node节点的树
        node:需要判断的节点的name值
        search:当检索到该节点时是否返回该节点的父节点和所有子节点
        ...
        if self.if_node_exist:
            ...
            pt(self.search_result_children)
            return True
        if tree.search_result_children == []:
            return False
        else:
            for child in tree.children:
                if child.name == name:
                    ...
                    self.if_node_name_exist_recursion(child, node_name, search)
