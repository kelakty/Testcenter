...
class SequencerThreadWorker(QObject):
    ...
    def run_sequence_table(self, test_sequence_list):
        for test_info in test_sequence_list:
            if test_info["DoIt"] == True:
                ...
                time.sleep(Timeout)
                #匹配log
                if self.matchlog(self.log_data_buffer) == True:
                    #匹配到log后
                    print("匹配到所需log")
    ...
    def matchlog(self,log_data_buffer):
        #匹配log
        if log_data_buffer.contains(GlobalVariable.Console[self.current_index]["MatchLog"]  ):
            return True
        else:
            return False
