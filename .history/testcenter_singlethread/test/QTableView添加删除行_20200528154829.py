...
class WindowClass(QMainWindow):
    ...
    def __init__(self,parent=None):
        ...
        # self.model=QStandardItemModel(4,4)#存储任意结构数据 仅能用于QTableView
        ...
        self.header = ['序号','姓名','年龄','地址']
        self.tableWidget.setHorizontalHeaderLabels(self.header)
        # self.tableWidget.setVerticalHeaderLabels(self.header)
        ...
        self.tool = self.addToolBar("File") #这里尝试使用QmenuBar，则此时会卡死，无法完成下面appedRow操作（猜测：可能是因为本身不允许menuBar完成这种操作）
        self.action= QAction("添加", self)
        self.action2=QAction("删除",self)
        self.tool.addAction(self.action)
        self.tool.addAction(self.action2)
        self.action.triggered.connect(lambda: self.tableWidget.insertColumn(self.tableWidget.columnCount())) #代表在第几列后面插入列
        self.action2.triggered.connect(lambda: self.tableWidget.removeColumn(self.tableWidget.CurrentChanged))
        # self.tool.actionTriggered[QAction].connect(self.processtrigger)
    ...
    #     if action.text()=="添加":
    #         print("开始添加")
    #         self.action.triggered.connect(self.tableWidget.insertColumn)
    #         # self.header.append([
    ...
    #         #     print(index.row())
    #         #     self.header.remove(index.row())
    #         self.action2.triggered.connect(self.tableWidget.insertRow)
