...
from sequencer import SequencerThread, SequencerThreadWorker
...
"selection-color: qconicalgradient(cx:0, cy:0, angle:135, stop:0 rgba(255, 255, 0, 69), stop:0.375 rgba(255, 255, 0, 69), stop:0.423533 rgba(251, 255, 0, 145), stop:0.45 rgba(247, 255, 0, 208), stop:0.477581 rgba(255, 244, 71, 130), stop:0.518717 rgba(255, 218, 71, 130), stop:0.55 rgba(255, 255, 0, 255), stop:0.57754 rgba(255, 203, 0, 130), stop:0.625 rgba(255, 255, 0, 69), stop:1 rgba(255, 255, 0, 69));")
        ...
        # 新建sequencer线程
        self.console_terminal_threadpool.append(SequencerThread())
        self.sequencer_worker = SequencerThreadWorker()   #需要传入当前激活的终端名
        self.sequencer_worker.moveToThread(self.console_terminal_threadpool[-1])
        self.console_terminal_threadpool[-1].started.connect(self.sequencer_worker.sequencer_init)
        self.sequencer_worker.seq_to_main_trigger.connect(self.sequencer_data_to_console_thread_slot, Qt.QueuedConnection)  #
        self.sequencer_worker.main_to_seq_trigger.connect(self.sequencer_worker.received_console_log)
...
#
    ...
    def on_open_close_buttom_clicked(self):
        ...
        #         #新建sequencer线程
        #         self.console_terminal_threadpool.append(SequencerThread())
        #         self.sequencer_worker = SequencerThreadWorker(GlobalVariable.Console[self.current_console_index]["consolethread"])   #需要传入当前激活的终端名
        #         self.sequencer_worker.moveToThread(self.console_terminal_threadpool[-1])
        #         self.console_terminal_threadpool[-1].started.connect(self.sequencer_worker.sequencer_init)
        #         self.sequencer_worker.seq_to_main_trigger.connect(self.sequencer_data_to_console_thread_slot, Qt.QueuedConnection)  #
        #         self.sequencer_worker.main_to_seq_trigger.connect(self.sequencer_worker.received_console_log)
        #         self.console_terminal_threadpool[-1].start() 
        #         print("当前sequencer线程：",self.sequencer_worker)
        #         print("当前sequencer_threadpool：",self.console_terminal_threadpool)
...
#######Sequencer测试序列功能########
    ...
    #             print("当前sequencer线程池：", self.sequencer_threadpool)
    #             print("当前serial线程池：", self.console_terminal_threadpool)
    #             self.sequencer_thread = SequencerThread(GlobalVariable.Console[self.current_console_index]["consolethread"])   #需要传入当前激活的终端名
    ...
    #             print("当前sequencer线程：",self.sequencer_thread)
    #             print("当前sequencer_threadpool[-1]：",self.sequencer_threadpool[-1])
    ...
    def sequencer_receive_data_slot(self, serialobj, data):
        self.sequencer_worker.main_to_seq_trigger.emit(serialobj, data)
