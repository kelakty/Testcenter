...
#######Sequencer测试序列功能########
    ...
    def on_run_test_sequence_clicked(self):
        ...
        """
        #获取当前激活的终端
        if self.mdiArea.currentSubWindow() != False:
            self.current_console_index = self.find_dictionarylist_keyvalue_index(GlobalVariable.Console, "subwindowobj", self.currentSubWindow())
            GlobalVariable.Console[self.current_console_index]
    ...
    def sequencer_data_to_console_thread_slot(self, data):
        #把Sequencer发过来的数据发送给对应console线程
        GlobalVariable.Console[self.current_console_index]["consolethread"].send_trigger.emit(data.encode(GlobalVariable.Console[index]["encodingtype"]))
