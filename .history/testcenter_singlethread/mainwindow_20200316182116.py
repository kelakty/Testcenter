...
#
    ...
    def newCom_MdiTread(self,com_treadcounter):  #新建Mdi页串口和新线程
        #创建一个MDIarea的sub窗口
        subwindow[com_treadcounter] = NewMdiSubWindow()
        subwindow[com_treadcounter].setMain(self)
        
        # 向sub内部添加QTextEdit控件
        subwindow[com_treadcounter].setWidget(QtWidgets.QTextEdit())
        # print("subwidget_objname:",sub.widget().objectName())
        subwindow[com_treadcounter].widget().insertPlainText("%d is %s\r\n" % (com_treadcounter,subwindow[com_treadcounter]))
        self.console_terminal=subwindow[com_treadcounter].widget()
        ...
        GlobalVariable.mdisubwindow_objlist.append(subwindow[com_treadcounter])
        GlobalVariable.opencom_objlist.append(self.console_terminal)  #####临时放这里！！！
        #对console_terminal进行事件过滤
        self.console_terminal.installEventFilter(self)

        subwindow[com_treadcounter].setWindowTitle(self.com_option.currentText())
        self.mdiArea.addSubWindow(subwindow[com_treadcounter])
        # objectname="subwindow"+str(self.count+1)
        # sub.setObjectName(objectname)
        subwindow[com_treadcounter].setAttribute(Qt.WA_DeleteOnClose) #设置subwindow属性，当点击关闭时删除对象
        subwindow[com_treadcounter].setWindowFlags(Qt.WindowTitleHint)
        subwindow[com_treadcounter].show()
