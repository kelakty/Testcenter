...
#
    ...
    def on_open_close_buttom_clicked(self):
        ...
        if self.com_option.currentText() in GlobalVariable.opencomlist:
            ...
            #创建新线程，初始化一个串口
            self.console_terminal_threadpool[0] = None
            self.console_terminal_threadpool[0] = SerialThread() #self.com_option.currentText()
            self.analyze=AnalyzObject()
            self.analyze.moveToThread(self.console_terminal_threadpool[0])
            self.console_terminal_threadpool[0].started.connect(self.analyze.serial_init)
            ...
            self.console_terminal_threadpool[0].start() 
