...
class AnalyzObject(QObject):
    ...
    def serial_init(self):
        ...
        try:
            self._serial[self.com_treadcounter] = QSerialPort()  # 用于连接串口的对象
        except Exception:
            self._serial.append(None)
            print("_serial列表对象是：",self._serial)
            self._serial[self.com_treadcounter] = QSerialPort() 
            print("_serial列表对象是：",self._serial[self.com_treadcounter])
        self._serial[self.com_treadcounter].setReadBufferSize(4096) #设置内部接收缓存区大小
        self._serial[self.com_treadcounter].readyRead.connect(self.onReadyRead)  # 绑定数据读取信号
        ...
        # 根据名字设置串口（也可以用上面的函数）
        self._serial[self.com_treadcounter].setPortName(port.systemLocation())
        # print(port.systemLocation())
        # 设置波特率
        self._serial[self.com_treadcounter].setBaudRate(  # 动态获取,类似QSerialPort::Baud9600这样的吧
            getattr(QSerialPort, 'Baud' + GlobalVariable.setting_baud_rate_option))
        # print(self._serial.baudRate())
        # 设置校验位
        self._serial[self.com_treadcounter].setParity(  # QSerialPort::NoParity
            getattr(QSerialPort, GlobalVariable.setting_checksum_bits + 'Parity'))
        # print(self._serial.parity())
        # 设置数据位
        self._serial[self.com_treadcounter].setDataBits(  # QSerialPort::Data8
            getattr(QSerialPort, 'Data' + GlobalVariable.setting_data_bits))
        # print(self._serial.dataBits())
        # 设置停止位
        self._serial[self.com_treadcounter].setStopBits(  # QSerialPort::Data8
        ...
        self._serial[self.com_treadcounter].setFlowControl(QSerialPort.NoFlowControl)
        # 读写方式打开串口
        ok = self._serial[self.com_treadcounter].open(QIODevice.ReadWrite) 
        # print(ok)
        if ok:
            #发送打开串口成功
            print("创建线程并打开串口成功")
    ...
    def sendto_comdata_slot(self,data):
        ...
        # print("发送信号槽函数:",data)
        self._serial[self.com_treadcounter].write(data)
    ...
    def onReadyRead(self):
        ...
        if self._serial[self.com_treadcounter].bytesAvailable():

            try:
                GlobalVariable.serialreaddata = self._serial[self.com_treadcounter].readAll() #self._serial.readLine()会出现转码错误，不知道为啥
                ...
                if GlobalVariable.serialreaddata != b'\x08 \x08':
                    if GlobalVariable.serialreaddata.contains(b'\r\r\n'):
                        ...
                        self.rec_trigger.emit(GlobalVariable.serialreaddata.data().decode('gb2312').replace("\r\r\n", "\r\n"))
                    #self.console_terminal.append(b'\x31\x32\x33'.decode('gb2312'))
                        ...
                        self.rec_trigger.emit(GlobalVariable.serialreaddata.data().decode('gb2312'))
                    #print('无退格')
