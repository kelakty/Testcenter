...
#                     self.analyze[com_threadcom].send_trigger.emit(input_s)
...
#                     #self.serial.write(input_s)
#                     self.analyze[com_threadcom].send_trigger.emit(input_s)
#                     terminal_cursor = self.console_terminal[com_threadcom].textCursor()#注意textCursor是一个类，因此要新建一个对象
...
#                     self.console_terminal[com_threadcom].setTextCursor(terminal_cursor)
#                     self.console_terminal[com_threadcom].cut()
# #                    
# #                    
...
#                     try:
#                         self.analyze[com_threadcom].send_trigger.emit(char.encode(encodingType))
#                     except Exception :
#                         print("发送字符信号发射出错")
...
#
    ...
    def on_open_close_buttom_clicked(self):
        ...
        if self.com_option.currentText() in GlobalVariable.opencomlist:
            ...
            try:
                if self.console_terminal_threadpool[GlobalVariable.com_threadcom] != None:
                    GlobalVariable.com_threadcom+=1
            ...
            if self.console_terminal_threadpool[GlobalVariable.com_threadcom] == None:
                print("开始创建com线程")
                ok=self.newCom_MdiTread(GlobalVariable.com_threadcom) 
                if ok == True:
                    GlobalVariable.com_threadcom+=1
                else:
                ...
                print("当前线程池不为空")
                self.console_terminal_threadpool.append(None)
                ok=self.newCom_MdiTread(GlobalVariable.com_threadcom+1) 
                if ok == True:
                    GlobalVariable.com_threadcom+=1
                else:
                    pass  #创建串口线程不成功，需要另外处理
    ...
    def newCom_MdiTread(self,com_threadcom):  #新建Mdi页串口和新线程
        #创建一个MDIarea的sub窗口
        try:
            self.subwindow[com_threadcom] = NewMdiSubWindow()
        except Exception:
            self.subwindow.append(None)
            self.subwindow[com_threadcom] = NewMdiSubWindow()
        self.subwindow[com_threadcom].setMain(self)
        
        # 向sub内部添加QTextEdit控件
        self.subwindow[com_threadcom].setWidget(QtWidgets.QTextEdit())
        # print("subwidget_objname:",sub.widget().objectName())
        self.subwindow[com_threadcom].widget().insertPlainText("%d is %s\r\n" % (com_threadcom,self.subwindow[com_threadcom]))
        
        try:
            self.console_terminal[com_threadcom]=self.subwindow[com_threadcom].widget()
        except Exception:
        ...
        self.console_terminal[com_threadcom].insertPlainText("self.consoleT_insert\r\n")
        

        GlobalVariable.mdisubwindow_objlist.append(self.subwindow[com_threadcom])
        GlobalVariable.opencom_objlist.append(self.console_terminal[com_threadcom])  #####临时放这里！！！
        #对console_terminal进行事件过滤
        self.console_terminal[com_threadcom].installEventFilter(self.subwindow[com_threadcom])

        self.subwindow[com_threadcom].setWindowTitle(self.com_option.currentText())
        self.mdiArea.addSubWindow(self.subwindow[com_threadcom])
        # objectname="subwindow"+str(self.count+1)
        # sub.setObjectName(objectname)
        self.subwindow[com_threadcom].setAttribute(Qt.WA_DeleteOnClose) #设置subwindow属性，当点击关闭时删除对象
        self.subwindow[com_threadcom].setWindowFlags(Qt.WindowTitleHint)
        self.subwindow[com_threadcom].show()
        ...
        #创建新线程，初始化一个串口
        # self.console_terminal_threadpool[com_threadcom] = None
        self.console_terminal_threadpool[com_threadcom] = SerialThread() #self.com_option.currentText()
        try:
            self.analyze[com_threadcom]=AnalyzObject(com_threadcom)
        except Exception:
            self.analyze.append(None)
            self.analyze[com_threadcom]=AnalyzObject(com_threadcom)
        self.analyze[com_threadcom].moveToThread(self.console_terminal_threadpool[com_threadcom])
        self.console_terminal_threadpool[com_threadcom].started.connect(self.analyze[com_threadcom].serial_init)
        self.analyze[com_threadcom].rec_trigger.connect(lambda: self.rec_comdata_slot(com_threadcom))  #,Qt.QueuedConnection
        self.analyze[com_threadcom].send_trigger.connect(self.analyze[com_threadcom].sendto_comdata_slot)
        self.console_terminal_threadpool[com_threadcom].start() 

        self.console_terminal[com_threadcom].selectionChanged.connect(lambda: self.console_terminal[com_threadcom].copy())  #选择文本自动复制
...
#                self.statusBar.showMessage(dis)
    ...
    def rec_comdata_slot(self,data,com_threadcom): #
        print("rec_comdata_slot com_threadcom is",com_threadcom,self.console_terminal)
        cursor = self.console_terminal[com_threadcom].textCursor()
        if(cursor != cursor.End):
            cursor.movePosition(cursor.End)
            self.console_terminal[com_threadcom].setTextCursor(cursor)

        self.console_terminal[com_threadcom].insertPlainText(data)
