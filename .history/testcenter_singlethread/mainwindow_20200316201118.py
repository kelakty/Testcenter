...
#
    ...
    def on_open_close_buttom_clicked(self):
        ...
        if self.com_option.currentText() in GlobalVariable.opencomlist:
            ...
            try:
                if self.console_terminal_threadpool[GlobalVariable.com_treadcounter] != None:
                    GlobalVariable.com_treadcounter+=1
            ...
            if self.console_terminal_threadpool[GlobalVariable.com_treadcounter] == None:
                print("开始创建com线程")
                ok=self.newCom_MdiTread(GlobalVariable.com_treadcounter) 
                if ok == True:
                    GlobalVariable.com_treadcounter+=1
                else:
                ...
                print("当前线程池不为空")
                self.console_terminal_threadpool.append(None)
                ok=self.newCom_MdiTread(GlobalVariable.com_treadcounter+1) 
                if ok == True:
                    GlobalVariable.com_treadcounter+=1
                else:
                    pass  #创建串口线程不成功，需要另外处理
    ...
    def newCom_MdiTread(self,com_treadcounter):  #新建Mdi页串口和新线程
        #创建一个MDIarea的sub窗口
        try:
            self.subwindow[com_treadcounter] = NewMdiSubWindow()
        except Exception:
            self.subwindow.append(None)
            self.subwindow[com_treadcounter] = NewMdiSubWindow()
        self.subwindow[com_treadcounter].setMain(self)
        
        # 向sub内部添加QTextEdit控件
        self.subwindow[com_treadcounter].setWidget(QtWidgets.QTextEdit())
        # print("subwidget_objname:",sub.widget().objectName())
        self.subwindow[com_treadcounter].widget().insertPlainText("%d is %s\r\n" % (com_treadcounter,self.subwindow[com_treadcounter]))
        
        try:
            self.console_terminal[com_treadcounter]=self.subwindow[com_treadcounter].widget()
        except Exception:
        ...
        self.console_terminal[com_treadcounter].insertPlainText("self.consoleT_insert\r\n")

        GlobalVariable.mdisubwindow_objlist.append(self.subwindow[com_treadcounter])
        GlobalVariable.opencom_objlist.append(self.console_terminal[com_treadcounter])  #####临时放这里！！！
        #对console_terminal进行事件过滤
        self.console_terminal[com_treadcounter].installEventFilter(self,com_treadcounter)

        self.subwindow[com_treadcounter].setWindowTitle(self.com_option.currentText())
        self.mdiArea.addSubWindow(self.subwindow[com_treadcounter])
        # objectname="subwindow"+str(self.count+1)
        # sub.setObjectName(objectname)
        self.subwindow[com_treadcounter].setAttribute(Qt.WA_DeleteOnClose) #设置subwindow属性，当点击关闭时删除对象
        self.subwindow[com_treadcounter].setWindowFlags(Qt.WindowTitleHint)
        self.subwindow[com_treadcounter].show()
        ...
        #创建新线程，初始化一个串口
        # self.console_terminal_threadpool[com_treadcounter] = None
        self.console_terminal_threadpool[com_treadcounter] = SerialThread() #self.com_option.currentText()
        try:
            self.analyze[com_treadcounter]=AnalyzObject(com_treadcounter)
        except Exception:
            self.analyze.append(None)
            self.analyze[com_treadcounter]=AnalyzObject(com_treadcounter)
        self.analyze[com_treadcounter].moveToThread(self.console_terminal_threadpool[com_treadcounter])
        self.console_terminal_threadpool[com_treadcounter].started.connect(self.analyze[com_treadcounter].serial_init)
        self.analyze[com_treadcounter].rec_trigger.connect(self.rec_comdata_slot,Qt.QueuedConnection)
        self.analyze[com_treadcounter].send_trigger.connect(self.analyze[com_treadcounter].sendto_comdata_slot)
        self.console_terminal_threadpool[com_treadcounter].start() 

        self.console_terminal[com_treadcounter].selectionChanged.connect(lambda: self.console_terminal[com_treadcounter].copy())  #选择文本自动复制
...
###工具栏按键###
    ...
    #事件过滤
    def eventFilter(self, obj, event,com_treadcounter):
        ...
        if event.type() == event.KeyPress:
            ...
            if obj in GlobalVariable.opencom_objlist:
                ...
                if event.key() == QtCore.Qt.Key_Up:
                    ...
                    self.analyze[com_treadcounter].send_trigger.emit(input_s)
                    ...
                    #self.serial.write(input_s)
                    self.analyze[com_treadcounter].send_trigger.emit(input_s)
                    terminal_cursor = self.console_terminal[com_treadcounter].textCursor()#注意textCursor是一个类，因此要新建一个对象
                    ...
                    self.console_terminal[com_treadcounter].setTextCursor(terminal_cursor)
                    self.console_terminal[com_treadcounter].cut()
#                    
#                    
...
#                        self.console_terminal.setTextCursor(cursor)
                    ...
                    #self.serial.write(char.encode(encodingType))
                    try:
                        self.analyze[com_treadcounter].send_trigger.emit(char.encode(encodingType))
                    except Exception :
                        print("发送字符信号发射出错")
...
#                self.statusBar.showMessage(dis)
    ...
    def rec_comdata_slot(self,data,rec_comdata_slot): #
        print("rec_comdata_slot com_treadcounter is",GlobalVariable.com_treadcounter,self.console_terminal)
        cursor = self.console_terminal[com_treadcounter].textCursor()
        if(cursor != cursor.End):
            cursor.movePosition(cursor.End)
            self.console_terminal[GlobalVariable.com_treadcounter].setTextCursor(cursor)

        self.console_terminal[GlobalVariable.com_treadcounter].insertPlainText(data)
