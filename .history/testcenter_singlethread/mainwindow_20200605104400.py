...
class NewMdiSubWindow(QtWidgets.QMdiSubWindow):
    ...
    def eventFilter(self, obj, event):
        ...
        if event.type() == event.KeyPress:
            #判断哪个窗口按下的事件
            # print("当前窗口对象是：",obj)
            if find_dictionarylist_keyvalue_index(GlobalVariable.Console, "console_terminal", obj) != False:
...
#
    ...
    def on_open_close_buttom_clicked(self):
        ...
        if self.find_dictionarylist_keyvalue_index(GlobalVariable.Console,"COM",self.com_option.currentText()) == False:
...
###工具栏按键###
    ...
    def rec_comdata_slot(self,threadobj,data): #
        print("rec_comdata_slot threadobj is",threadobj)
        cursor = self.console_terminal.textCursor()
        if(cursor != cursor.End):
            cursor.movePosition(cursor.End)
            self.console_terminal.setTextCursor(cursor)
