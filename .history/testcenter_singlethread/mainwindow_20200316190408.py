...
#
    ...
    def newCom_MdiTread(self,com_treadcounter):  #新建Mdi页串口和新线程
        ...
        self.analyze.moveToThread(self.console_terminal_threadpool[com_treadcounter])
        self.console_terminal_threadpool[com_treadcounter].started.connect(self.analyze.serial_init)
        self.analyze.rec_trigger.connect(self.rec_comdata_slot,Qt.QueuedConnection)
        self.analyze.send_trigger.connect(self.analyze.sendto_comdata_slot)
        self.console_terminal_threadpool[com_treadcounter].start() 

        self.console_terminal.selectionChanged.connect(lambda: self.console_terminal.copy())  #选择文本自动复制
