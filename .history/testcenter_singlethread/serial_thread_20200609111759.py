...
class SerialConsoleThread(Console):   #QObject
    ...
    def serial_init(self):
        ...
        if GlobalVariable.mainwindow.savelogdir.text() == "":
            # print("savelogdir是空的")
            datetime = "_%d%02d%02d_%d_%02d_%02d" % (date.year, date.month, date.day,time.hour,time.minute,time.second)
            GeneralLogger().set_log_path('/test.log')  
        else:
            save_log_path = GlobalVariable.mainwindow.savelogdir.text()
            GeneralLogger().set_log_path
