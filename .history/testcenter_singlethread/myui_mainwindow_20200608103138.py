...
#        self.dockWidget.setAllowedAreas(QtCore.Qt.RightDockWidgetArea | QtCore.Qt.LeftDockWidgetArea)#设置dockWidget只停靠在左边栏
        ...
        t = time.strftime("%d%02d%02d_%d_%02d_%02d"% (date.year, date.month, date.day,time.hour,time.minute,time.second))
        # t=time.strftime("%Y-%m-%d %H:%M:%S")
        self.lcd.display(t)
        # self.mainWindow.lcdtimer = Qtimer(self)
        # self.mainWindow.lcdtimer.start()
        # self.mainWindow.lcdtimer.timeout.connect(self.clock)
