...
class NewMdiSubWindow(QtWidgets.QMdiSubWindow):
    ...
    def closeEvent(self, event):
        ...
        if(result == QtWidgets.QMessageBox.Yes):
            ...
            # GlobalVariable.serial[].close #对串口进行关闭操作
            ...
            self.mainwindow.console_terminal_threadpool[self.com_threadCounter].quit()
            self.mainwindow.console_terminal_threadpool[self.com_threadCounter].wait()
            self.mainwindow.console_terminal_threadpool[self.com_threadCounter].deleteLater()
            self.mainwindow.analyze.pop()
            self.mainwindow.console_terminal_threadpool.pop()
            self.mainwindow.subwindow.pop()
            GlobalVariable.comThreadCounter-=1
            print("线程池：",self.mainwindow.console_terminal_threadpool)
            print("线程对象:",self.mainwindow.analyze)
            print("子窗口：",self.mainwindow.subwindow)
            print("计数：",GlobalVariable.comThreadCounter)
            print("串口线程已关闭")
        else:
            event.ignore()
