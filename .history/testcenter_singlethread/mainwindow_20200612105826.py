    ...
    def on_run_test_sequence_clicked(self):
        ...
        if self.mdiArea.currentSubWindow() != None:
            ...
            if self.current_console_index >= 0:
                ...
                sequencer_thread = SequencerThreadWorker()   #需要传入当前激活的终端名
                sequencer_thread.moveToThread(self.sequencer_threadpool)
                self.sequencer_threadpool.started.connect( sequencer_thread.sequencer_init)
                sequencer_thread.seq_to_main_trigger.connect(self.sequencer_data_to_console_thread_slot, Qt.QueuedConnection)  #
                sequencer_thread.main_to_seq_trigger.connect(sequencer_thread.received_console_log)
