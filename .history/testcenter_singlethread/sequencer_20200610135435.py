...
class SequencerThreadWorker(QObject):
    ...
    def sequencer_init(self):
        ...
        #     time.sleep(2)
        #     print("while里log缓存区：", self.log_data_buffer)
    ...
    def run_sequence_table(self, test_sequence_list):
        for test_info in test_sequence_list:
            if test_info["DoIt"] == True:
                ...
                time.sleep(Timeout)
                print("log缓存区：", self.log_data_buffer)
                #匹配log
                if self.matchlog(test_info,self.log_data_buffer) == True:
                    #匹配到log后
                    print("匹配到所需log")
    ...
    def received_console_log(self, serialobj, data):
        ...
        print("log缓存：", self.log_data_buffer)
