    ...
    def on_open_close_buttom_clicked(self):
        ...
        if self.mdiArea.currentSubWindow() != None:
            ...
            if self.current_console_index >= 0:
                ...
                #新建sequencer线程
                self.sequencer_thread = SequencerThread()
                self.sequencer_worker = SequencerThreadWorker(GlobalVariable.Console[self.current_console_index]["consolethread"])   #需要传入当前激活的终端名
                self.sequencer_worker.moveToThread(self.sequencer_thread)
                self.sequencer_thread.started.connect(self.sequencer_worker.sequencer_init)
                ...
                print("当前sequencer_thread：",self.sequencer_thread)
