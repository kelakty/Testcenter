    ...
    def on_open_close_buttom_clicked(self):
        ...
        if self.find_dictionarylist_keyvalue_index(GlobalVariable.Console, "name", self.com_option.currentText()) < 0:
            if self.tryopencom() == True: #尝试打开串口，串口没打开则新建串口线程
                ...
                #sequencer相关的信号槽
                GlobalVariable.Console[index]["consolethread"].rec_trigger.connect(self.sequencer_receive_data_slot)  #串口线程中发送信号过来，通过sequencer相关的槽函数进行接收
...
#######Sequencer测试序列功能########
    ...
    def on_run_test_sequence_clicked(self):
        ...
        self.sequencer_thread.rec_trigger.connect(self.rec_comdata_slot, Qt.QueuedConnection)  #
        self.sequencer_thread.send_trigger.connect(sequencer_thread.sendto_comdata_slot)
