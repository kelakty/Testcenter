...
class CRT():
    ...
    def __init__(self): #,  parent=None
        ...
        print('%-25s: %s, %s,' % ("scriptCRTfunc_init", QThread.currentThread(), int(QThread.currentThreadId())))
        print('%-25s: %s, %s,' % ("scriptCRTfunc_init", threading.current_thread().name, threading.current_thread().ident))
           

    def Send(self,sendstring):
        # print("send命令发送的字符串",sendstring)
        GlobalVariable.mainwindow.analyze.send_trigger.emit(sendstring.encode(encodingType))
    ...
    def SendKeys(self,sendkey):
        GlobalVariable.mainwindow.analyze.send_trigger.emit(sendkey.encode(encodingType))
