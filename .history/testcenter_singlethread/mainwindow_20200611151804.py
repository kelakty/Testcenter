...
"selection-color: qconicalgradient(cx:0, cy:0, angle:135, stop:0 rgba(255, 255, 0, 69), stop:0.375 rgba(255, 255, 0, 69), stop:0.423533 rgba(251, 255, 0, 145), stop:0.45 rgba(247, 255, 0, 208), stop:0.477581 rgba(255, 244, 71, 130), stop:0.518717 rgba(255, 218, 71, 130), stop:0.55 rgba(255, 255, 0, 255), stop:0.57754 rgba(255, 203, 0, 130), stop:0.625 rgba(255, 255, 0, 69), stop:1 rgba(255, 255, 0, 69));")
        ...
        #临时测试
        tablelist_header = []
        # tablelist_header.append(self.sequencertable.horizontalHeader.count())
        # print("Header:",self.sequencertable.horizontalHeader.count())
        print("Header:",self.sequencertable.horizontalHeader.hiddenSectionCount())
        print("Header:",self.sequencertable.horizontalHeader.length())
        print("Header:",self.sequencertable.horizontalHeader.saveState())
...
#######Sequencer测试序列功能########
    ...
    def on_run_test_sequence_clicked(self):
        ...
        if self.mdiArea.currentSubWindow() != None:
            ...
            # self.sequencertable.
            self.tablelist = [[0 for i in range(self.sequencertable.rowCount())] for j in range(self.sequencertable.columnCount())]
            for i in range(self.sequencertable.columnCount()):
                for j in range(self.sequencertable.rowCount()):
                    print("表格内容",self.sequencertable.item(j,i).text())
                    self.tablelist[i][j] = self.sequencertable.item(j,i).text()
            tablelist_header = self.sequencertable.horizontalHeader()
            ...
            for i in self.tablelist:
                ...
                self.table_dict_list.updata(table_oneline_dict)
            ...
            if self.current_console_index >= 0:
                ...
                self.sequencer_threadpool[-1].started.connect(self.sequencer_thread.sequencer_init(self.table_dict_list))
