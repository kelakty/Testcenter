...
class SerialConsoleThread(QObject):   #QObject
    ...
    def serial_init(self):
        ...
        serial = QSerialPort()  # 用于连接串口的对象
        print("serial对象是：",serial)
        serial.setReadBufferSize(4096) #设置内部接收缓存区大小
        serial.readyRead.connect(self.onReadyRead)  # 绑定数据读取信号

        # 根据配置连接串口
        port = GlobalVariable.SelectCom  
        print(port)

        # 根据名字设置串口（也可以用上面的函数）
        serial.setPortName(port.systemLocation())
        
        # 设置波特率
        serial.setBaudRate(  # 动态获取,类似QSerialPort::Baud9600这样的吧
        ...
        # 设置停止位
        serial.setStopBits(  # QSerialPort::Data8
    ...
    def onReadyRead(self):
        ...
        if serial.bytesAvailable():
            try:
                GlobalVariable.serialreaddata = serial.readAll() #self._serial.readLine()会出现转码错误，不知道为啥
