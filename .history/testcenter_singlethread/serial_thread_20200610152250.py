            ...
            try:
                if GlobalVariable.serialreaddata != b'\x08 \x08':
                    if GlobalVariable.serialreaddata.contains(b'\r\r\n'):
                        data = GlobalVariable.serialreaddata.data().decode(encoding=self.encodingtype,errors="ignore").replace("\r\r\n", "\r\n")
                    ...
                    #self.console_terminal.append(b'\x31\x32\x33'.decode('gb2312'))
                        ...
                        data = GlobalVariable.serialreaddata.data().decode(encoding=self.encodingtype,errors="ignore")
                        self.rec_trigger.emit(self.serial,data)
                ...
                #ﾾ
                data = GlobalVariable.serialreaddata.data().decode(encoding="utf-8", errors="ignore")
                self.rec_trigger.emit(self.serial,data)
