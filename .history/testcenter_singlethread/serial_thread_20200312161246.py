...
class SerialThread(QThread):
    ...
    def __init__(self):
        ...
        if ok:
            #发送打开串口成功
            print("创建线程并打开串口成功")
            print("当前的线程ID是：",self.currentThreadId())
    ...
    def run(self):
        ...
        # 循环完毕后发出信号	
        a="run函数运行结束发射出信号"	
        self.rec_trigger.emit(a)

    def sendto_comdata_slot(self,data):
        
        # print("发送信号槽函数:",data)
        self._serial.write(data)
        print('%-25s: %s, %s,' % ("SerialThread_senddata_slot", QThread.currentThread(), int(QThread.currentThreadId())))
        print('%-25s: %s, %s,' % ("SerialThread_senddata_slot", threading.current_thread().name, threading.current_thread().ident))
         

    def onReadyRead(self):
        # 数据接收响应
        print('%-25s: %s, %s,' % ("SerialThread_onReadyRead_slot", QThread.currentThread(), int(QThread.currentThreadId())))
...
class AnalyzObject(QObject):
    ...
    def sendto_comdata_slot(self,data):
        ...
        # print("发送信号槽函数:",data)
        self._serial.write(data)
        print('%-25s: %s, %s,' % ("AnalyzObject_senddata", QThread.currentThread(), int(QThread.currentThreadId())))
        print('%-25s: %s, %s,' % ("AnalyzObject_senddata", threading.current_thread().name, threading.current_thread().ident))
