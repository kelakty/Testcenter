...
#
    ...
    def on_open_close_buttom_clicked(self):
        ...
        if self.com_option.currentText() in GlobalVariable.opencomlist:
            ...
            self.send_trigger.connect(self.analyze.sendto_comdata_slot)
...
#                        self.console_terminal.setTextCursor(cursor)
                    ...
                    #self.serial.write(char.encode(encodingType))
                    try:
                        self.send_trigger.emit(char.encode(encodingType))
                    except Exception :
                        print("发送字符信号发射出错")
                    ...
                    self.send_trigger.emit("test_run_init".encode(encodingType))
