...
#
    ...
    def on_open_close_buttom_clicked(self):
        ...
        if GlobalVariable.Console == [] or find_dictionarylist_keyvalue_index(
                    GlobalVariable.Console,"COM",self.com_option.currentText()) != False:
            if self.tryopencom() == True: #尝试打开串口，串口没打开则新建串口线程
                ...
                #对终端设置window标题
                if self.renameconsole != "":
                    subwindow.setWindowTitle(self.renameconsole)
                else:
