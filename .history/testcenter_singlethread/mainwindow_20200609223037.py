...
#######Sequencer测试序列功能########
    ...
    def on_run_test_sequence_clicked(self):
        ...
        """
        #获取当前激活的终端
        if self.currentSubWindow() != False:
            index = self.find_dictionarylist_keyvalue_index(GlobalVariable.Console, "subwindowobj", self.currentSubWindow())
            GlobalVariable.Console[index]

            #串口线程中发送信号过来，通过sequencer相关的槽函数进行接收
            GlobalVariable.Console[index]["consolethread"].rec_trigger.connect(self.sequencer_receive_data_slot, Qt.QueuedConnection) 


        #新建sequencer线程
        self.sequencer_threadpool.append(QThread())
        self.sequencer_thread=SequencerThread()   #需要传入当前激活的终端名
        ...
        self.sequencer_thread.seq_to_main_trigger.connect(self.sequencer_data_to_main_slot, Qt.QueuedConnection)  #
    ...
    def sequencer_data_to_main_slot(self, data):
        
        #把Sequencer发过来的数据发送给对应console线程
        GlobalVariable.Console[index]["consolethread"].send_trigger.emit(data.encode(GlobalVariable.Console[index]["encodingtype"]))
    def sequencer_data_from_main_to_console_thread(self)
        pass
