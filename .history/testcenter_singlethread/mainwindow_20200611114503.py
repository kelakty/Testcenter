...
#######Sequencer测试序列功能########
    ...
    def on_import_to_sequencer_clicked(self):
        ...
        if choosefilename:
            ...
            print("测试序列表格是：\r\n",self.test_sequence)
        # for i in range(len(self.test_sequence.loc[:,'发送指令'].values)):
        print("index:",len(self.test_sequence.index))
        print("columns:",len(self.test_sequence.columns))
        print("item is :",self.test_sequence.iloc[0,3])
        print("item is :",self.test_sequence.iloc[0,2])
        for i in range(len(self.test_sequence.index)):
            for j in range(len(self.test_sequence.columns)):
                if self.test_sequence.iloc[i,j] != None:
                    print(i,j,self.test_sequence[i][j])
                    save=str(self.test_sequence[i][j])
                    newItem = QTableWidgetItem(save)
                    self.sequencertable.setItem(i,j,newItem)
        ...
        self.sequencertable.setItem(0,0,QTableWidgetItem("save"))
        self.sequencertable.setItem(1,1,QTableWidgetItem("save"))
        print(self.sequencertable.rowCount(),self.sequencertable.columnCount())

        # headerlist = ["第一","第二","第三","第四"]
        # self.sequencertable.setHorizontalHeaderLabels(headerlist)
        # for i in range(len(3)):
        #     self.sequencertable.setHorizontalHeaderItem(3,"356")
