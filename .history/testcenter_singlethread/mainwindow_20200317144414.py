...
class NewMdiSubWindow(QtWidgets.QMdiSubWindow):
    ...
    def sendcounter(self,comThreadCounter):
        self.comThreadCounter=comThreadCounter
...
#                     self.analyze[comThreadCounter].send_trigger.emit(input_s)
...
#                     #self.serial.write(input_s)
#                     self.analyze[comThreadCounter].send_trigger.emit(input_s)
#                     terminal_cursor = self.console_terminal[comThreadCounter].textCursor()#注意textCursor是一个类，因此要新建一个对象
...
#                     self.console_terminal[comThreadCounter].setTextCursor(terminal_cursor)
#                     self.console_terminal[comThreadCounter].cut()
# #                    
# #                    
...
#                     try:
#                         self.analyze[comThreadCounter].send_trigger.emit(char.encode(encodingType))
#                     except Exception :
#                         print("发送字符信号发射出错")
...
#
    ...
    def newCom_MdiTread(self,comThreadCounter):  #新建Mdi页串口和新线程
        ...
        self.analyze[comThreadCounter].rec_trigger.connect(lambda: self.rec_comdata_slot(comThreadCounter))  #,Qt.QueuedConnection
        ...
        self.subwindow[comThreadCounter].sendanalyze_arg(self.analyze[comThreadCounter])
...
#                self.statusBar.showMessage(dis)
    ...
    def rec_comdata_slot(self,data,comThreadCounter): #
