...
class LogAnalyzer(QWidget, Ui_loganalyzer):
    ...
    def logextract(self):
        ...
        self.savefilepathname+="_%d%02d%02d_%d_%02d_%02d"% (date.year, date.month, date.day,time.hour,time.minute,time.second)+".xlsx"
        savelogfile=open(self.savefilepathname,'a+')

        #提取查找到文字的后面第几行： lineEdit_saveline
        savelinenum=self.lineEdit_saveline.split(',')
        savelinenum.sort() 
        ...
        while True:
            ...
            if find1!=[]:
                #保存匹配到的那一行
                openlogfile.write(data)
                for i in int(maximumlinenum):
