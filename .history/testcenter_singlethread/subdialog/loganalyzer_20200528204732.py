...
class LogAnalyzer(QWidget, Ui_loganalyzer):
    ...
    def logextracttoexcel(self):
        ...
        self.savefilepathname=self.logfilepathname.replace('.log', '')
        self.savefilepathname=self.logfilepathname.replace('.txt', '')
        self.savefilepathname+="_%d%02d%02d_%d_%02d_%02d"% (date.year, date.month, date.day,time.hour,time.minute,time.second)+".excel"
        savelogfile=open(self.savefilepathname,'a+')
    ...
    def on_pushButton_gen_2_clicked(self):
        ...
        """
        contentlist = [[0 for i in range(self.tableWidget.columnCount())] for j in range(self.tableWidget.rowCount())]
        contentstr = []

        #判断表格至少有一列信息，每列信息都有分号分割，否则返回错误提示
        print("表格行列数：",self.tableWidget.columnCount(), self.tableWidget.rowCount())
        for i in range(self.tableWidget.rowCount()):
            for j in range(self.tableWidget.columnCount()):
                print("表格内容",self.tableWidget.item(i,j).text())
                contentlist[i].append(self.tableWidget.item(i,j).text())
        print("contentlist",contentlist)
        # for i in range(len(contentlist)):
        # columnname, matchstr= map(str,contentlist[0].split("；"))
        # print(columnname,matchstr)
