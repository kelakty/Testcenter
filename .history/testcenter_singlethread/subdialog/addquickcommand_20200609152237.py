...
class AddQuickCommand(QDialog, Ui_AddQuickCommand):
    ...
    def addquickcommand(self):
        ...
        if quickcommand != "":     #此处用None并不能真正的拦截输入为空
            ...
            setobjectname_command="self.mainwindow.action"+str(
                    GlobalVariable.quickcommand_number+1)+".setObjectName(\"action"+str(
                    GlobalVariable.quickcommand_number+1)+ "\")"
            exec(setobjectname_command)
            toolbar_addaction="self.mainwindow.toolBar_quickcommand.addAction(
                    self.mainwindow.action"+str(GlobalVariable.quickcommand_number+1)+")"
            exec(toolbar_addaction)
            toolbar_settext="self.mainwindow.action"+str(GlobalVariable.quickcommand_number+1
                    )+".setText(QtCore.QCoreApplication.translate(\"MainWindow\",\""+GlobalVariable.quickcommand_namelist[len(
                        GlobalVariable.quickcommand_namelist)-1]+"\"))"
            ...
            toolbar_command_trigger="self.mainwindow.action"+str(GlobalVariable.quickcommand_number+1
                )+".triggered.connect(partial(self.mainwindow.actiontoolbar_sendcommand_triggered,GlobalVariable.quickcommand_list[len(GlobalVariable.quickcommand_list)-1]))"
            ...
            toolbar_hoveraction="self.mainwindow.action"+str(GlobalVariable.quickcommand_number+1
                )+".hovered.connect(partial(self.mainwindow.action_hoverd,self.mainwindow.action"+str(
                    GlobalVariable.quickcommand_number+1)+"))"
