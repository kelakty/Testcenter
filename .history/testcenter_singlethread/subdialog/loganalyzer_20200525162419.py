...
class LogAnalyzer(QWidget, Ui_loganalyzer):
    ...
    def logextract(self):
        ...
        logfile=logfilepath.replace("\\","/")
        openlogfile=open(logfile,encoding='UTF-8')

        #生成要保存的log文件
        savefile=self.lineEdit_savepath.text() + "\\result.txt"
        file=savefile.replace("\\","/")
        savelogfile=open(file,'a+')
