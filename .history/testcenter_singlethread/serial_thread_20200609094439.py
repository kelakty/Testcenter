...
class SerialConsoleThread(Console):   #QObject
    ...
    def __init__(self,com_option_name):  #parent=None
        ...
        # print('%-25s: %s, %s,' % ("AnalyzeObject_init", QThread.currentThread(), int()))
    ...
    def serial_init(self):
        ...
        #初始化待保存log处理的中间全局变量
        GlobalVariable.waiting_to_send.update({str(QThread.currentThreadId()) : {"senddata":[], "residue":""}})
        print("waitingtosend是：",GlobalVariable.waiting_to_send)
    ...
    def sendto_comdata_slot(self,data):
        ...
        GlobalVariable.waiting_to_send.update(self.log_post_process.log_joint_and_decode(
            str(QThread.currentThreadId()), GlobalVariable.waiting_to_send, data, 'gb2312'))
        for i in GlobalVariable.waiting_to_send[str(QThread.currentThreadId())]["senddata"]:
            ...
            try:
                ...
                if GlobalVariable.serialreaddata != b'\x08 \x08':
                    if GlobalVariable.serialreaddata.contains(b'\r\r\n'):
                        data = GlobalVariable.serialreaddata.data().decode('gb2312').replace("\r\r\n", "\r\n")
                        self.rec_trigger.emit(self.serial,data)

                        GlobalVariable.waiting_to_receive.update(self.log_post_process.log_joint_and_decode(
                            str(QThread.currentThreadId()), GlobalVariable.waiting_to_receive, data, 'gb2312'))
                        for i in GlobalVariable.waiting_to_receive[str(QThread.currentThreadId())]["senddata"]:
                            self.thread_logger.info(i)
                        # self.thread_logger.info(data)
                    #self.console_terminal.append(b'\x31\x32\x33'.decode('gb2312'))
                        ...
                        data = GlobalVariable.serialreaddata.data().decode('gb2312')
                        self.rec_trigger.emit(self.serial,data)
