        ...
        if event.type() == event.KeyPress:
            ...
            if obj in GlobalVariable.opencom_objlist:
                ...
                if event.key() == QtCore.Qt.Key_Up:
                    ...
                    self.analyze[GlobalVariable.treadcounter].send_trigger.emit(input_s)
                    ...
                    #self.serial.write(input_s)
                    self.analyze[GlobalVariable.treadcounter].send_trigger.emit(input_s)
                    terminal_cursor = self.console_terminal[GlobalVariable.treadcounter].textCursor()#注意textCursor是一个类，因此要新建一个对象
                    ...
                    self.console_terminal[GlobalVariable.treadcounter].setTextCursor(terminal_cursor)
                    self.console_terminal[GlobalVariable.treadcounter].cut()
#                    
#                    
...
#                        self.console_terminal.setTextCursor(cursor)
                    ...
                    #self.serial.write(char.encode(encodingType))
                    try:
                        self.analyze[GlobalVariable.treadcounter].send_trigger.emit(char.encode(encodingType))
                    except Exception :
                        print("发送字符信号发射出错")
...
#                self.statusBar.showMessage(dis)
    ...
    def rec_comdata_slot(self,data): #
        cursor = self.console_terminal[GlobalVariable.treadcounter].textCursor()
        if(cursor != cursor.End):
            cursor.movePosition(cursor.End)
            self.console_terminal[GlobalVariable.treadcounter].setTextCursor(cursor)

        self.console_terminal[GlobalVariable.treadcounter].insertPlainText(data)
