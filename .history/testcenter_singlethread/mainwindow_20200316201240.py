...
#
    ...
    def newCom_MdiTread(self,com_treadcounter):  #新建Mdi页串口和新线程
        ...
        self.analyze[com_treadcounter].rec_trigger.connect(lambda: self.rec_comdata_slot(com_treadcounter))  #,Qt.QueuedConnection
...
#                self.statusBar.showMessage(dis)
    ...
    def rec_comdata_slot(self,data,com_treadcounter): #
        print("rec_comdata_slot com_treadcounter is",com_treadcounter,self.console_terminal)
        cursor = self.console_terminal[com_treadcounter].textCursor()
        if(cursor != cursor.End):
            cursor.movePosition(cursor.End)
            self.console_terminal[com_treadcounter].setTextCursor(cursor)

        self.console_terminal[com_treadcounter].insertPlainText(data)
